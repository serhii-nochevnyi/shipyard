...
// @invariant: since/mergeable_since derive from nowIso — excluded so the YAML is a pure fn of GitHub state
const YAML_VOLATILE_FIELDS = new Set(['since', 'mergeable_since']);
const yaml = [];
for (const id of Object.keys(state).sort()) {
  const s = canonicalValue(state[id]);
  yaml.push(`${JSON.stringify(id)}:`);
  for (const [k, v] of Object.entries(s)) {
    if (YAML_VOLATILE_FIELDS.has(k)) continue;
    yaml.push(`  ${k}: ${JSON.stringify(v)}`);
  }
}
...
const published = withLock(lockDirFor(ROOT), 'tracker-record', () => withLock(lockDirFor(ROOT), 'state', () => {
  ...
  // @invariant: no `#` line — generation rides delivery-state-meta.json and the stdout line below instead
