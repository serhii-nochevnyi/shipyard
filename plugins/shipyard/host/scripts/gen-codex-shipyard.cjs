...
function shipyardRewrites(text, scriptsRoot) {
  ...
  return process.env.SHIPYARD_CODEX_MARKETPLACE === '1'
    ? converted.replace(/\$shipyard-(route|investigate|decompose|deliver|bench|delivery-rules)\b/g,
      '$shipyard:shipyard-$1')
    : converted;
}
...
module.exports = { readCodexCliCapabilities, shipyardRewrites, codexStaticVariants, validateCodexBundle, DEEP_ROLES, DEEP_SUFFIX, CRITICAL_ROLES, CRITICAL_SUFFIX };
