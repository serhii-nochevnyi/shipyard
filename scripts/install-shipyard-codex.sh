...
const block = codexBlock(phase, process.env.SHIPYARD_CODEX_MARKETPLACE === '1');
...
if [[ "${SHIPYARD_CODEX_MARKETPLACE:-0}" == 1 ]]; then
  echo "  In Codex: \$shipyard:shipyard-route | \$shipyard:shipyard-investigate | \$shipyard:shipyard-decompose | \$shipyard:shipyard-deliver | \$shipyard:shipyard-bench"
else
