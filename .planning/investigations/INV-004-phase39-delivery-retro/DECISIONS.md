...
## Phase 40 planning refinements (2026-09-24)
**Why:** Open questions raised by the phase 40 researcher (`40-RESEARCH.md` "Open Questions").
**What was rejected:** automatic `.gitignore` writes by bootstrap; keeping `epic/<phase-dir>` names in target projects; a separate cheap smoke grid (an ADR-014 change); a `gate_status:` trailer in PR bodies.
**Scope fence:** Target projects make `.planning/` untracked through an explicit, confirmed one-time migration command, and the REQ-133 warning is inverted for target projects (it fires when `.planning/` is tracked); target-project epic branches are `feat/<slug>` or `feat/<JIRA-KEY>-<slug>` with neutral epic PR title and body, phase mapping kept in the graph; commit subjects are in hygiene scope; gate verdicts move to a commit status; the live round uses each role's base rung of its runtime grid; the live receipt lives under `~/.local/state/shipyard/live/` keyed by version and tree sha and is fresh for 7 days; host provenance is a sidecar keyed by `dispatch_id`, not a receipt field; `models.*` tuning keys are written only for Claude.
