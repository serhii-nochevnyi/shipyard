...
- `make test-live` runs one real research → decompose → executor → sentinel round per runtime on an in-repo fixture project with each role's base rung of its runtime grid, and the release script refuses without a fresh passing live receipt.
