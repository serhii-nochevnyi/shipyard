...
Verification commands are **scoped to `files_modified`** — the test files/filters
covering the change plus typecheck and lint over the touched paths, runnable in a
bare worktree with no external services. They are not a project-wide `npm test`:
the executor runs them on every attempt and the fix roles on every babysit round,
across tickets running in parallel, so their cost is the conveyor's tick rate.
Checks that need a live service, a browser, or a deployed environment are CI-only
and are recorded under Test strategy instead.
...
- **GSD projection gates** (capability v0.54.0): `gsd-sync.cjs` writes after
