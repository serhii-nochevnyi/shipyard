...
- `pipeline-config.cjs` — the single reader for `.planning/config.json` → `pipeline`, plus the role × risk × attempt model policy (`model <role> [flags]`). **It only ever emits the tier aliases `opus`/`sonnet`/`haiku`, because the Agent tool validates `model` against exactly that set** — a full model ID or a suffixed alias like `opus[1m]` is rejected on input. Unknown config keys and non-alias model values produce warnings rather than silent no-ops.
