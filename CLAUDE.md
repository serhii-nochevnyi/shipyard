...
**Overlay image installs gsd-core and registers plugins.** The overlay runs `npx --yes @opengsd/gsd-core@<version> --claude --global --profile=full` during build, which writes Claude Code plugin configuration under `~/.claude`. The andrej-karpathy-skills plugin is staged via `scripts/sync-karpathy-skills.sh` (clones at a pinned commit `KARPATHY_SKILLS_REF`) into `.build/karpathy-skills/` and registered with `claude plugin`. The in-repo `pipeline` plugin (`plugins/delivery-pipeline/`) is copied to `/opt/delivery-pipeline` and registered the same way; it implements the delivery conveyor from `docs/gsd_multilevel_delivery_pipeline.md` (`/pipeline:investigate`, `/pipeline:decompose`, `/pipeline:deliver` plus deterministic scripts under `plugins/delivery-pipeline/scripts/`). The companion GSD capability (`capabilities/delivery-pipeline/`) is installed at global scope via `gsd-tools capability install`; it contributes a blocking `plan:post` gate (`command-exit-zero` → the bundled `validate-graph.cjs`), making the conveyor's Gate 2 a mechanical part of the GSD loop. Four additional plugins are installed from the official `claude-plugins-official` marketplace (`anthropics/claude-plugins-official`): `skill-creator`, `code-simplifier`, `github` (GitHub MCP server), and `typescript-lsp` (TypeScript/JS LSP via `typescript-language-server`). Plugin versions from the official marketplace are pinned by the marketplace's GitHub ref at clone time. The `host ~/.config/gh` directory is mounted read-only into the container at `/home/dev/.config/gh` so the GitHub CLI (`gh`) can use host authentication.
